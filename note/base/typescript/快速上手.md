...
虽然 TypeScript 最终是被编译成 JavaScript 执行的，但它依然是一门全新的语言。从学习的角度来讲，存在着不小的成本。这里仅说明它有哪些特点，在后续的章节中，我们在详细深入讲解它的语法。
...
如果是单纯的定义语法，那 TypeScript 实际上是做不了什么的，所以必须要有与之配套的工具。

- 编译器：将 TypeScript 代码编译成 JavaScript 代码。
- 语言服务：提供类型检查、代码补全、代码导航、重构等功能。
- 工程配置：配置 TypeScript 项目的编译选项、输出目录、模块系统等。

##### 生态融合层

世界已经是 JS 的了，TS 必须兼容整个世界”
